...
const TableOne = ({item}) => {
  ...
  return (
    <div className="w-full group rounded mb-10 border border-indigo-500 transition-all duration-500">
        <div className="w-full center justify-between bg-indigo-500 group-hover:bg-transparent rounded p-4 flex flex-row justify-between items-center border-b border-indigo-500 cursor-pointer" onClick={setDisplayFunction}>
            <h2 className='text-white group-hover:text-indigo-500 text-xl transition-all duration-500'>{title}</h2>
            <div className={`${display[0] === "hidden" && "rotate-180"}  text-white group-hover:text-indigo-500 text-xl transition-all duration-200`}>&#9650;</div>
